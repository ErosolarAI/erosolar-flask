...
## Version 0.3.0 - Workflow Commands & UI (2024-12-XX)

### 🚦 Claude Code Slash Commands
- Added `/feature-dev` to mirror the seven-phase Claude Code feature workflow (discovery through summary) with explicit user approvals at each gated phase.
- Added `/commit`, `/commit-push-pr`, `/clean_gone`, and `/code-review` commands that replicate the Anthropic plugins for git automation, PR creation, branch cleanup, and confidence-scored reviews.

### 🖥️ Improved Agent UX
- Workflow activations now render a dedicated status panel in the CLI highlighting the active goal and phase list.
- Non-verbose sessions include a **Tool Trace** panel summarizing the latest tool invocations for quick auditing.
- Added warnings for unknown slash commands plus a `/workflow_clear` escape hatch so you can exit an overlay without restarting the agent.

### 🔌 Internal Plumbing
- Conversation management now routes all slash commands through a shared router + workflow manager so CLI and API clients behave identically.
- Notifications are queued and drained after each turn, making it trivial for future front-ends to mirror the Claude Code UX cues.
