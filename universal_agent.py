...
from claude_integration import WorkflowManager, SlashCommandRouter
...
# --------- Hook-aware ToolNode wrapper ----------

class HooksAwareToolNode:
    """Wrapper around ToolNode that executes hooks before and after tool calls."""
    ...
    def __call__(self, state: MessagesState) -> Dict[str, List[BaseMessage]]:
        """Execute tools with hook support."""
        ...
        # Process each tool call with hooks
        ...
        for tool_call in ai_message.tool_calls:
            ...
            allowed, pre_messages = self.hooks_manager.run_pre_tool_hooks(tool_name, tool_input)

            if pre_messages:
                hook_warnings.extend(pre_messages)

            if not allowed:
                # Hook blocked the tool call
                from langchain_core.messages import ToolMessage

                error_msg = "\n".join(pre_messages) if pre_messages else "Tool call blocked by security hook"
                tool_messages.append(ToolMessage(
                    content=f"[BLOCKED BY HOOK] {error_msg}",
                    tool_call_id=tool_call.get("id", ""),
                ))
                continue

            # Execute the actual tool via base_node
            # We need to create a temporary state with just this tool call
            temp_ai_msg = AIMessage(
                content="",
                tool_calls=[tool_call],
            )
            temp_state = {"messages": messages[:-1] + [temp_ai_msg]}

            # Execute tool
            result = self.base_node(temp_state)

            # Get the tool result messages
            result_messages = result.get("messages", [])

            # Run PostToolUse hooks on results
            for msg in result_messages:
                if getattr(msg, "type", None) == "tool":
                    tool_output = getattr(msg, "content", "")
                    modified_output, post_messages = self.hooks_manager.run_post_tool_hooks(
                        tool_name,
                        tool_input,
                        tool_output,
                    )
                    ...
                    if post_messages:
                        hook_warnings.extend(post_messages)

                    # Note: We currently don't modify output, but could here
                    tool_messages.append(msg)
                else:
                    tool_messages.append(msg)

        # Display hook warnings to user if any
        if hook_warnings:
            for warning in hook_warnings:
                cli_ui.print_status(f"[hook] {warning}", kind="warning")

        return {"messages": tool_messages}


tool_node = HooksAwareToolNode(TOOLS)  # Executes tools with hook support
...
def planning_node(state: MessagesState):
    ...
    # Check if we're in plan mode (stored in state metadata)
    plan_mode_active = state.get("plan_mode", False)

    if plan_mode_active:
        # Use interactive planner
        planner_messages = create_planner_messages(
            ensure_message_content(last_user.content),
            AgentMode.PLAN
        )
        if planner_messages:
            planner_response = planner.invoke(planner_messages)
            interactive_plan = parse_interactive_plan(planner_response)

            if interactive_plan:
                # Store the interactive plan in the message
                plan_dict = {
                    "mode": interactive_plan.mode,
                    "steps": interactive_plan.steps,
                    "questions": [
                        {
                            "id": q.id,
                            "question": q.question,
                        ...
                        for q in interactive_plan.questions
                    ],
                }
                plan_msg = AIMessage(
                    name="planner",
                    content=ensure_message_content(plan_dict),
                    additional_kwargs={"plan": plan_dict, "interactive": True},
                )
                return {"messages": [plan_msg]}

    # Default planning (execution mode)
...
class ConversationManager:
    ...
    def __init__(self, compiled_graph):
        ...
        self.plan_mode_manager = PlanModeManager()
        self.command_router = SlashCommandRouter(self.workflow_manager, self.plan_mode_manager)
    ...
    def locked_submit(self, content: str, source: str = "cli"):
        ...
        with self._lock:
            user_payload = ensure_message_content(content)
            new_messages = self.command_router.transform(user_payload, source)
            if new_messages:
                for msg in new_messages:
                    self.history.append(msg)
                reply_start = len(self.history)

                # Add plan_mode flag to state if in plan mode
                state = {"messages": self.history}
                if self.plan_mode_manager.is_plan_mode():
                ...
                # Check if we got an interactive plan that needs questions answered
                if self.plan_mode_manager.is_plan_mode():
                    for msg in responses:
                        if (getattr(msg, "type", None) == "ai" and
                            getattr(msg, "name", "") == "planner"):
                            additional = getattr(msg, "additional_kwargs", {})
                            if additional.get("interactive"):
                                # This is an interactive plan - store it
                                interactive_plan = parse_interactive_plan(msg)
                                if interactive_plan:
                                    self.plan_mode_manager.create_plan(
                                        mode=interactive_plan.mode,
                                        steps=interactive_plan.steps,
                                        questions=interactive_plan.questions
                                    )
            else:
                responses = []
        yield responses
...
def _collect_tool_trace(messages, limit: int = 4) -> List[str]:
    entries: List[str] = []
    for msg in messages:
        if getattr(msg, "type", None) != "tool":
            continue
        name = getattr(msg, "name", "tool")
        excerpt = stringify_content(getattr(msg, "content", ""))
        if isinstance(excerpt, str):
            compressed = " ".join(excerpt.split())
        else:
            compressed = str(excerpt)
        if len(compressed) > 160:
            compressed = compressed[:157] + "..."
        entries.append(f"{name}: {compressed}")
    if not entries:
        return []
...
def _print_tool_trace(messages):
    trace = _collect_tool_trace(messages)
    if not trace:
        return
    cli_ui.print_panel("Tool Trace", trace, style="tool")
...
def display_responses(messages, *, streamer=None, session_id=None, verbose=None):
    ...
    # Check for interactive plan in messages
    for msg in messages:
        if (getattr(msg, "type", None) == "ai" and
            getattr(msg, "name", "") == "planner"):
            additional = getattr(msg, "additional_kwargs", {})
            if additional.get("interactive"):
                # Display the interactive plan nicely
                plan_data = additional.get("plan", {})
                cli_ui.print_interactive_plan(plan_data, show_details=True)
                return
...
def _drain_workflow_notifications(conversation: ConversationManager):
    manager = getattr(conversation, "workflow_manager", None)
    if not manager:
        return
    notes = manager.pop_notifications()
    for note in notes:
        if note.kind == "panel":
            cli_ui.print_panel(note.title or "Workflow", note.body, style="info")
        else:
            cli_ui.print_status(note.body, kind=note.kind)
...
def run_cli_chat(conversation: ConversationManager, stop_event: threading.Event):
    ...
    cli_ui.print_status(
        "Workflow commands: /feature-dev, /commit, /commit-push-pr, /clean-gone, /code-review, /workflow-clear",
        kind="info",
    )
    cli_ui.print_status(
        "Plan mode: /plan-mode (interactive), /execution-mode (default), /show-plan, /answer <id>:<choice>, /execute-plan",
        kind="info",
    )
    CLI_READY.set()
        ...
        while not stop_event.is_set():
            ...
            try:
                with conversation.locked_submit(user_text, source="cli") as responses:
                    ...
                    _drain_workflow_notifications(conversation)
            except Exception as exc:  # noqa: BLE001
...
def main():
    ...
    for hook_config in plugin_loader.hook_configs:
        hooks_manager.register_hooks_from_config(hook_config)

    # Report loaded plugins
    plugins = plugin_loader.list_plugins()
    if plugins:
        cli_ui.print_status(f"Loaded {len(plugins)} plugin(s): {', '.join(p.name for p in plugins)}", kind="info")

    commands = plugin_loader.list_commands()
    if commands:
        cli_ui.print_status(f"Available slash commands: {', '.join('/' + c for c in commands[:5])}{' and more...' if len(commands) > 5 else ''}", kind="info")
